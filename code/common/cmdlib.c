...
#define	BASEDIRNAME		"base"	// important that these 2 are in this order!!!!
#define	BASEDIRNAME2	"game"	//
...
char		baseDirName[64] = {BASEDIRNAME};
...
static qboolean SetQdirFromPath2( const char *path, const char *psBaseDir )
{
	const char	*c;
	const char *sep;
	int		len, count;
...
void SetQdirFromPath( const char *path )
{
	if (SetQdirFromPath2( path, baseDirName )	// try software version, in case it was overridden
		||
		SetQdirFromPath2( path, BASEDIRNAME )	// ... else try the two hardwired ones
		||
		SetQdirFromPath2( path, BASEDIRNAME2 )	// actually, this last isn't needed inhouse, but will work for stuff like "game/demo" instead of "game/base" (I think). Whatever.
		)
	{
		return;
	}
...
int Q__stricmp (const char *s1, const char *s2)
{
